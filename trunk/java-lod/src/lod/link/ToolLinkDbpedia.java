...
import lod.common.MyConfig;
import lod.common.MyLogger;
...
public class ToolLinkDbpedia {
	...
	private static File getFile(String filepath){
		return MyConfig.getFile(MyConfig.PATH_DIV_DATA, filepath);
	}
	...
	public static int name2dbpediaByFile(File fileInput, File fileOutput,   Collection<String> knownNames,  Collection<String> stopWords){
		MyLogger.println(String.format("process name2dbpedia: input file %s --- output file %s", fileInput.getAbsolutePath(), fileOutput.getAbsolutePath()));
		...
		try {
			...
			MyLogger.println(String.format("input %d entries", cnt_total));
			MyLogger.println(String.format("processed %d entries", cnt_processed));
			MyLogger.println(String.format("verified %d best mappings", cnt_verified));
			return cnt_verified;
	...
	public static  DataSmartMap name2dbpedia(String name, Collection<String> stopWords){
		...
		try {
			...
			for (int i=0; i<Math.min(aryUrl.size(),MAX_WIKIPEDIA_URL); i++){
				...
				if (null== m){
					szTempLink =null;
					MyLogger.println("cannot load dbpedia data");
					continue;
				}
				...
				if (m.listObjectsOfProperty(m.createResource(szTempLink),m.createProperty("http://dbpedia.org/ontology/wikiPageDisambiguates")).hasNext()){
					szTempLink =null;
					MyLogger.println("encounter a disambiguation page");
					continue;					
				}
				...
				if (!verifyMapping(name, m, stopWords)){
					MyLogger.println("cannot verify the mapping");
					MyLogger.println(szTempLink);
					szTempLink =null;
				}else{
	...
	//	}
		...
		for (String [] entry : landmark){
			if (entry[1].indexOf(myname)>=0){
				MyLogger.println(String.format("Matched by [%s] --- \t input:%s; \t output:%s",  entry[0], myname, entry[1] ));
				return true;				
			}
		}
		return false;
	...
	public static  String name2homepage(String name){
		...
		try {
			...
			if (szTempTitle.toLowerCase().indexOf(name.toLowerCase())<0){
				MyLogger.println(String.format("result:%b \t input:%s; \t output:%s", szTempTitle.toLowerCase().indexOf(name.toLowerCase())>=0, name,szTempTitle ));
				MyLogger.println(szTempLink);
				return null;
			}
